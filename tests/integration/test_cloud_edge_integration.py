...
class TestCloudEdgeIntegration(unittest.TestCase):
    ...
    def test_task_creation_and_assignment(self):
        ...
        async def run_test():
            # 注册节点
            self.db.add_node("task-node", {
                "node_id": "task-node",
                "status": "online"
            })

            # 注册设备
            self.db.add_device("task-device", {
...
class TestTaskLifecycle(unittest.TestCase):
    ...
    def test_complete_task_lifecycle(self):
        ...
        async def run_test():
            ...
            self.db.add_device("lifecycle-device", {"device_id": "lifecycle-device", "host": "localhost"})
