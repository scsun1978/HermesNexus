...
class TestRollbackAPI:
    ...
    def test_create_rollback_plan_success(self, client, mock_auth_headers, sample_rollback_plan_data):
        """测试成功创建回滚计划"""
        # 模拟认证和权限检查
        with patch('cloud.api.rollback_api.get_current_user', return_value={
            "user_id": "test-user-001",
            "roles": ["tenant_admin"]
        }):
    ...
    def test_create_rollback_plan_forbidden(self, client, mock_auth_headers, sample_rollback_plan_data):
        """测试权限不足创建回滚计划"""
        # 模拟普通用户，没有管理员权限
        with patch('cloud.api.rollback_api.get_current_user', return_value={
            "user_id": "test-user-001",
    ...
    def test_create_rollback_plan_invalid_type(self, client, mock_auth_headers, sample_rollback_plan_data):
        """测试无效的回滚类型"""
    ...
    def test_get_rollback_plan_success(self, client, mock_auth_headers):
        ...
        with patch('cloud.api.rollback_api.get_current_user', return_value={
            ...
            with patch('cloud.api.rollback_api.get_rollback_service') as mock_service:
                ...
                response = client.get(
                    "/api/v1/rollback/plans/rollback-test001",
                    headers=mock_auth_headers
                )
    ...
    def test_list_rollback_plans_with_filters(self, client, mock_auth_headers):
        """测试带过滤条件列出回滚计划"""
        mock_plans = [
            MagicMock(plan_id="rollback-001", status="planned")
        ]
    ...
    def test_create_failure_record_invalid_type(self, client, mock_auth_headers, sample_failure_data):
        """测试无效的故障类型"""
    ...
    def test_create_recovery_plan_success(self, client, mock_auth_headers, sample_recovery_plan_data):
        """测试成功创建恢复计划"""
    ...
    def test_create_recovery_plan_invalid_action(self, client, mock_auth_headers, sample_recovery_plan_data):
        """测试无效的恢复动作"""
        invalid_data = {**sample_recovery_plan_data, "recovery_action": "invalid_action"}
    ...
    async def test_full_rollback_workflow_api(self, client, mock_auth_headers):
        ...
        with patch('cloud.api.rollback_api.get_current_user', return_value={
            ...
            get_response = client.get(
                f"/api/v1/rollback/plans/{plan_id}",
                headers=mock_auth_headers
            )
