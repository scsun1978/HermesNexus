#!/usr/bin/env python3
"""HermesNexus 完整版 Cloud API - 包含任务结果回写"""
import http.server
import socketserver
import json
import sqlite3
...
class HermesAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if parsed_path.path == '/health':
                ...
                self.send_nodes()
            elif parsed_path.path.startswith('/api/v1/nodes/') and '/tasks' in parsed_path.path:
                # 处理节点任务查询
                parts = parsed_path.path.split('/')
                if len(parts) >= 5 and parts[4] == 'tasks':
                    node_id = parts[3]
                    if len(parts) == 5:  # /api/v1/nodes/{node_id}/tasks
                        self.send_node_tasks(node_id)
                    elif len(parts) > 5:  # /api/v1/nodes/{node_id}/tasks/{task_id}
                        task_id = parts[5]
                        self.send_node_task_detail(node_id, task_id)
    ...
    def do_POST(self):
        ...
        try:
            ...
            elif parsed_path.path.endswith('/result'):
                # 处理任务结果回写: /api/v1/nodes/{node_id}/tasks/{task_id}/result
                parts = parsed_path.path.split('/')
                # 修复URL解析bug
                if len(parts) >= 7 and parts[6] == 'result':
                    node_id = parts[4]  # 修正: 应该是parts[4]
                    task_id = parts[6]  # 修正: 应该是parts[6]
                    result_data = json.loads(post_data.decode('utf-8'))
                    self.handle_task_result(node_id, task_id, result_data)
            else:
                self.send_not_found()
    ...
    def send_health_check(self):
        ...
        response = {
            ...
            "deployment": "complete-api-with-result-writeback"
        }
    ...
    def send_node_tasks(self, node_id):
        """获取节点的任务列表"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()

        cursor.execute(
            "SELECT task_id, node_id, task_type, status, created_at FROM tasks WHERE node_id = ? ORDER BY created_at DESC LIMIT 50",
            (node_id,)
        )
        rows = cursor.fetchall()
    ...
    def send_node_task_detail(self, node_id, task_id):
        """获取节点任务详情"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()

        cursor.execute(
            "SELECT task_id, node_id, task_type, target, status, result, created_at, completed_at FROM tasks WHERE task_id = ? AND node_id = ?",
            (task_id, node_id)
        )
        row = cursor.fetchone()
    ...
    def create_task(self, task_data):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                f"event-{int(datetime.now().timestamp())}-{task_id}",
                "task_created",
                "task",
                task_id,
                json.dumps({"task_type": task_data.get("task_type"), "node_id": task_data.get("node_id")}),
                current_time
            ))
        ...
        try:
            ...
            try:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
                    f"audit-{int(datetime.now().timestamp())}-{task_id}",
...
def init_database():
    ...
    conn.commit()
    conn.close()
    print("✅ 数据库初始化完成 (包含events和audit_logs表)")

def main():
    """主函数"""
    print("🚀 HermesNexus 完整版 Cloud API 启动中...")
