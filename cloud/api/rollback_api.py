...
from shared.models.rollback import (
    ...
    FailureType, FailureSeverity, RollbackStatistics
)
...
def _plan_to_dict(plan: Any) -> Dict[str, Any]:
    ...
    for step in steps:
        if isinstance(step, BaseModel):
            ...
            step_data = {}
        step_data["rollback_type"] = _enum_or_value(step_data.get("rollback_type", getattr(step, "rollback_type", None)))
        step_data["status"] = _enum_or_value(step_data.get("status", getattr(step, "status", RollbackStatus.PLANNED)))
...
async def execute_rollback_plan(
    ...
    try:
        ...
        plan = await service.execute_rollback_plan(
            ...
            auto_confirm=request_data.auto_confirm
        )
...
async def list_rollback_plans(
    ...
    try:
        ...
        plans = service.list_rollback_plans(
            status=status_enum,
            rollback_type=rollback_type_enum,
            limit=limit
        )
...
async def rollback_health_check():
    ...
    return {
        ...
        "active_recoveries": len(get_recovery_service()._active_recoveries) if hasattr(get_recovery_service(), '_active_recoveries') else 0
    }