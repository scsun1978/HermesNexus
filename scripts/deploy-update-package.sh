...
echo "=== 创建Cloud API v1.2.0更新部署包 ==="
echo ""

# 创建临时目录
DEPLOY_DIR="deploy-cloud-api-update"
rm -rf "$DEPLOY_DIR"
mkdir -p "$DEPLOY_DIR"
...
# 复制更新的Cloud API文件
cp cloud/api/v12_standard_cloud.py "$DEPLOY_DIR/"

# 复制更新脚本
cp scripts/update-cloud-api.sh "$DEPLOY_DIR/"
...
echo ""
echo "🚀 部署命令:"
echo "  1. 上传到服务器: scp -r $DEPLOY_DIR scsun@172.16.100.101:/home/scsun/"
echo "  2. 登录服务器: ssh scsun@172.16.100.101"
echo "  3. 执行更新: cd /home/scsun/$DEPLOY_DIR && ./update-cloud-api.sh"
echo ""
log_success "部署包已准备就绪！"