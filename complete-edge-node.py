#!/usr/bin/env python3
"""HermesNexus 完整版 Edge 节点 - 支持任务结果回写"""
import json
import time
import subprocess
import requests
    ...
    def poll_tasks(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.cloud_api_url}/api/v1/nodes/{self.node_id}/tasks",
                timeout=5
            )
            if response.status_code == 200:
    ...
    def write_back_result(self, task_id, result):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"   结果已接收: {resp_data.get('result_received')}")
            else:
...
def main():
    ...
    print("🚀 HermesNexus 完整版 Edge 节点")
    print("================================")
    print("🔥 新功能: 任务结果回写到Cloud API")
